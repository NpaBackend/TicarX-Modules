...
- Added option to publish stubs for modules `php artisan vendor:publish --provider="TicarX\Modules\LaravelModulesServiceProvider" --tag="stubs"`
...
- Fixed bug: `Target class [TicarX\Modules\Commands\] does not exist.`
...
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\TicarX\Modules\Module`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\TicarX\Modules\Module`.
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\TicarX\Modules\FileRepository`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\TicarX\Modules\FileRepository`.
- **BREAKING** Removed the `__get` magic method on the `\TicarX\Modules\Module` class. Use `get()` or `json()->get()` instead.
